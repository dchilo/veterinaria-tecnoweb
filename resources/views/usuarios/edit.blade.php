        ...
        <div class="container-fluid py-4 px-0">
            <div class="card">
                ...
                <div class="card-body pt-4 p-3">
                    <form action="{{ route('usuarios.update', $user->id) }}" method="POST" role="form text-left">
                        @csrf
                        @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="@error('user.name')border border-danger rounded-3 @enderror">
                                        <input class="form-control" value="{{ $user->name }}" type="text"
                                            placeholder="Nombre" id="user-name" name="name" required>
                                        @error('name')
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="@error('email')border border-danger rounded-3 @enderror">
                                        <input class="form-control" value="{{ $user->email }}" type="email"
                                            placeholder="@example.com" id="user-email" name="email" required>
                                        @error('email')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="@error('user.password')border border-danger rounded-3 @enderror">
                                        <input class="form-control" value="" type="password"
                                            placeholder="Contraseña" id="user-password" name="password" minlength="8">
                                        @error('password')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label for="tipo_usuario" class="form-control-label">Tipo Usuario</label>
                                    <div class="@error('tipo_usuario') border border-danger rounded-3 @enderror">
                                        <select class="form-control" name="tipo_usuario" id="tipo_usuario" required>
                                            <option value="" disabled {{ $user->tipo_usuario ? '' : 'selected' }}>Seleccione</option>
                                            <option value="Técnico" {{ $user->tipo_usuario == 'Técnico' ? 'selected' : '' }}>Técnico</option>
