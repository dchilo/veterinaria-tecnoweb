    ...
    <div>
        ...
        <div class="d-flex justify-content-between mb-4" style="margin-right: 8px; position: relative;">
            ...
            <div class="mt-4 mb-3" style="position: absolute; top: 8px; right: 0;">
                <a href="{{ route('motorizados.create') }}" class="btn bg-gradient-dark btn-md">Nuevo</a>
                
                <a href="{{ route('motorizados.generatePDF') }}" class="btn bg-gradient-dark btn-md">PDF</a>
            </div>
        </div>
                ...
                <div class="card mb-4 mx-0">
                    ...
                    <div class="card-body" style="padding: 1rem">
                        <div class="table-responsive p-0">
                            <table id="motorizadosTable" class="table align-items-center mb-0">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    @foreach ($motorizados as $motorizado)
                                        <tr>
                                            <td class="ps-4">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->id }}</p>
                                            </td>

                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->cliente->name }}
                                                </p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->modelo }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->marca }}</p>
                                            </td>
                                            
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->anio }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->placa }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $motorizado->estado }}</p>
                                            </td>

                                            <td class=" text-center">


                                                <form action="{{ route('motorizados.destroy', $motorizado) }}"
                                                    method="post">
                                                    @method('delete')
                                                    @csrf

                                                    <a href="{{ route('motorizados.edit', $motorizado->id) }}"
                                                        class="text-white" data-bs-toggle="tooltip"
                                                        data-bs-original-title="Editar motorizado">
                                                        <i class="fas fa-pencil-alt text-secondary"></i>
                                                    </a>

                                                    {{-- <a href="{{ route('motorizados.show', $motorizado->id) }}"
                                                        class="mx-3 text-white" data-bs-toggle="tooltip"
                                                        data-bs-original-title="Ver motorizado">
                                                        <i class="fas fa-eye text-secondary"></i>
                                                    </a> --}}

                                                    <button type="submit" class="cursor-pointer"
                                                        style="border: none; background: none;"
                                                        onclick="return confirm('¿Está seguro de que desea borrar este motorizado?');">
