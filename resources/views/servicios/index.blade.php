    ...
    <div>
        ...
        <div class="d-flex justify-content-between mb-4" style="margin-right: 8px; position: relative;">
            ...
            <div class="mt-4 mb-3" style="position: absolute; top: 8px; right: 0;">
                <a href="{{ route('servicios.create') }}" class="btn bg-gradient-dark btn-md">Nuevo</a>
                <a href="{{ route('servicios.generatePDF') }}" class="btn bg-gradient-dark btn-md">PDF</a>
            </div>
        </div>
                ...
                <div class="card mb-4 mx-0">
                    ...
                    <div class="card-body pb-2" style="padding: 0.8rem">
                        <div class="table-responsive p-0">
                            <table id="serviciosTable" class="table align-items-center ">
                                <thead>
                                    <tr>
                                ...
                                <tbody>
                                    @foreach ($servicios as $servicio)
                                        <tr>
                                            <td class="ps-4">
                                                <p class="text-xs font-weight-bold mb-0">{{ $servicio->id }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $servicio->nombre }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $servicio->descripcion }}</p>
                                            </td>
                                            <td class="text-center">
                                                <p class="text-xs font-weight-bold mb-0">{{ $servicio->precio }} Bs. </p>
                                            </td>
                                            <td class=" text-center">
                                                <form action="{{ route('servicios.destroy', $servicio) }}" method="post">
                                                    ...
                                                    <a href="{{ route('servicios.edit', $servicio->id) }}"
                                                        data-bs-toggle="tooltip" data-bs-original-title="Editar servicio">
                                                        <i class="fas fa-pencil-alt text-secondary"></i>
                                                    </a>
                                                    <button type="submit" class="cursor-pointer"
                                                        style="border: none; background: none;"
                                                        onclick="return confirm('¿Está seguro de que desea borrar este servicio?');">
