        ...
        <div class="container-fluid py-4 px-0">
            <div class="card">
                ...
                <div class="card-body pt-4 p-3">
                    <form action="{{ route('promociones.store') }}" method="POST" role="form text-left">
                        @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="@error('descuento') border border-danger rounded-3 @enderror">
                                        <input class="form-control" type="number" step="0.01" placeholder="Descuento" id="promocion-descuento"
                                            name="descuento" value="{{ old('descuento') }}" required>
                                        @error('descuento')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <div class="@error('descripcion') border border-danger rounded-3 @enderror">
                                        <textarea class="form-control" id="promocion-descripcion" name="descripcion" rows="3">{{ old('descripcion') }}</textarea>
                                        @error('descripcion')
                                            <p class="text-danger text-xs mt-2">{{ $message }}</p>
                                        @enderror
                                    </div>
